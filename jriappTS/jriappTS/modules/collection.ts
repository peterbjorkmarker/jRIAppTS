module RIAPP {
    export module MOD {
        export module collection {
           ...
           export interface IPermissions { canAddRow: bool; canEditRow: bool; canDeleteRow: bool; canRefreshRow: bool; };
           ...
           export class CollectionItem extends RIAPP.BaseObject implements binding.IErrorNotification, MOD.utils.IEditable, MOD.utils.ISubmittable {
                ...
                _beginEdit(): bool {
                    var coll = this._collection, isHandled:bool;
                    if (coll.isEditing) {
           ...
           export interface ICollectionOptions {
                enablePaging: bool; pageSize: number;
            };

           export interface ICollChangedArgs { change_type: string; items: CollectionItem[]; pos?: number[]; old_key?: string; new_key?: string; }
           export interface ICollFillArgs { isBegin: bool; rowCount: number; time: Date; isPageChanged: bool; resetUI?: bool; fetchedItems?: CollectionItem[]; newItems?: CollectionItem[]; }
           export interface ICollValidateArgs { item: CollectionItem; fieldName: string; errors: string[]; }
           export interface ICollItemStatusArgs { item: CollectionItem; oldChangeType: number; key: string; }
           export interface ICollItemAddedArgs { item: CollectionItem; isAddNewHandled: bool; }


           export class Collection extends RIAPP.BaseObject {
                _options: ICollectionOptions;
                _isLoading: bool;
                _EditingItem: CollectionItem;
                _perms: IPermissions;
                _totalCount: number;
                _pageIndex: number;
                _items: CollectionItem[];
                _itemsByKey: { [key: string]: CollectionItem; };
                _currentPos: number;
                _newKey: number;
                _fieldMap: { [fieldName: string]: IFieldInfo; };
                _errors: { [item_key: string]: { [fieldName:string]: string[]; }; };
                _ignoreChangeErrors: bool;
                _pkInfo: IFieldInfo[];
                _isUpdating: bool;
                ...
                addOnItemAdding(fn: (sender: Collection, args: { item: CollectionItem; isCancel: bool; }) => void , namespace?: string) {
                    this.addHandler('item_adding', fn, namespace);
                }
                removeOnItemAdding(namespace?: string) {
                    this.removeHandler('item_adding', namespace);
                }
                addOnItemAdded(fn: (sender: Collection, args: { item: CollectionItem; isAddNewHandled: bool; }) => void , namespace?: string) {
                    this.addHandler('item_added', fn, namespace);
                }
                removeOnItemAdded(namespace?: string) {
                    this.removeHandler('item_added', namespace);
                }
                addOnCurrentChanging(fn: (sender: Collection, args: { newCurrent: CollectionItem; }) => void , namespace?: string) {
                ...
                addOnPageChanging(fn: (sender: Collection, args: { page: number; isCancel: bool; }) => void , namespace?: string) {
                ...
                addOnErrorsChanged(fn: (sender: Collection, args: { item: CollectionItem; }) => void , namespace?: string) {
                    this.addHandler('errors_changed', fn, namespace);
                }
                removeOnErrorsChanged(namespace?: string) {
                    this.removeHandler('errors_changed', namespace);
                }
                addOnBeginEdit(fn: (sender: Collection, args: { item: CollectionItem; }) => void , namespace?: string) {
                    this.addHandler('begin_edit', fn, namespace);
                }
                removeOnBeginEdit(namespace?: string) {
                    this.removeHandler('begin_edit', namespace);
                }
                addOnEndEdit(fn: (sender: Collection, args: { item: CollectionItem; isCanceled: bool; }) => void , namespace?: string) {
                    this.addHandler('end_edit', fn, namespace);
                }
                removeOnEndEdit(namespace?: string) {
                    this.removeHandler('end_edit', namespace);
                }
                addOnCommitChanges(fn: (sender: Collection, args: { item: CollectionItem; isBegin: bool; isRejected: bool; changeType: number; }) => void , namespace?: string) {
                ...
                addOnStatusChanged(fn: (sender: Collection, args: ICollItemStatusArgs) => void , namespace?: string) {
                ...
                _onCurrentChanging(newCurrent: CollectionItem) {
                    try {
                        this.endEdit();
                    } catch (ex) {
                ...
                _onEditing(item: CollectionItem, isBegin:bool, isCanceled:bool) {
                    if (this._isUpdating)
                        return;
                    if (isBegin) {
                ...
                _onCommitChanges(item: CollectionItem, isBegin: bool, isRejected: bool, changeType: number) {
                ...
                _onItemStatusChanged(item: CollectionItem, oldChangeType: number) {
                    this.raiseEvent('status_changed', { item: item, oldChangeType: oldChangeType, key: item._key });
                }
                _validateItem(item: CollectionItem): MOD.binding.IValidationInfo {
                ...
                _validateItemField(item: CollectionItem, fieldName: string): MOD.binding.IValidationInfo {
                ...
                _addErrors(item: CollectionItem, errors: MOD.binding.IValidationInfo[]) {
                    var self = this;
                ...
                _addError(item: CollectionItem, fieldName: string, errors: string[]) {
                    if (!fieldName)
                        fieldName = '*';
                ...
                _removeError(item: CollectionItem, fieldName:string) {
                    var itemErrors = this._errors[item._key];
                    if (!itemErrors)
                        return;
                    if (!fieldName)
                ...
                _removeAllErrors(item: CollectionItem) {
                ...
                _getErrors(item: CollectionItem): { [fieldName: string]: string[]; } {
                    return this._errors[item._key];
                }
                _onErrorsChanged(item: CollectionItem) {
                    var args = { item: item };
                    this.raiseEvent('errors_changed', args);
                    item._onErrorsChanged(args);
                }
                _onItemDeleting(item) {
                    var args = { item: item, isCancel: false };
                    this.raiseEvent('item_deleting', args);
                    return !args.isCancel;
                }
                _onFillStart(args: { isBegin: bool; rowCount: number; time: Date; isPageChanged: bool; }) {
                    this.raiseEvent('fill', args);
                }
                _onFillEnd(args: { isBegin: bool; rowCount: number; time: Date; isPageChanged: bool; resetUI: bool; fetchedItems: CollectionItem[]; newItems: CollectionItem[]; }) {
                    this.raiseEvent('fill', args);
                }
                _onItemsChanged(args: { change_type: string; items: CollectionItem[]; pos?: number[]; old_key?: string; new_key?: string; }) {
                    this.raiseEvent('coll_changed', args);
                }
                //new item is being added, but is not in the collection now
                _onItemAdding(item: CollectionItem) {
                    var args = { item: item, isCancel: false };
                ...
                _onItemAdded(item: CollectionItem) {
                ...
                _attach(item: CollectionItem, itemPos?:number) {
                ...
                _onRemoved(item: CollectionItem, pos: number) {
                    try {
                ...
                _setCurrentItem(v: CollectionItem) {
                ...
                getItemsWithErrors() {
                    var self = this, res:CollectionItem[]=[];
                ...
                addNew() {
                    var item: CollectionItem, isHandled:bool;
                ...
                forEach(callback:(item:CollectionItem)=>void, thisObj?:any) {
                ...
                waitForNotLoading(callback, callbackArgs:any[], syncCheck:bool, groupName:string) {
           ...
           export class List extends Collection {
                ...
                constructor(type_name:string, properties) {
                    super();
                ...
                _createNew() {
                    var item:CollectionItem = new this._itemType(null);
                ...
                _createItemType() {
                    ...
                    this._itemType = MOD.utils.__extendType(ListItem, {
                        __coll: this,
                        toString: function () {
                ...
                fillItems(objArray:any[], clearAll?:bool) {
           ...
           export class Dictionary extends List {
                _keyName: string;
                constructor(type_name:string, properties, keyName:string) {
                    if (!keyName)
