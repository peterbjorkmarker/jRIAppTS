...
The framework was designed for achieving maximum convenience and performance, and for this sake it works in browsers which support ECMA Script 5.1 level of javascript.
Those browsers include Internet Explorer 9 and above, Mozilla Firefox 4+, Google Chrome 13+, and Opera 11.6+. 
...
The domain model is used on the client and generated by code generation feature of the DataService (as typescript code).
...
A DataForm - the control which bounds a datacontext to a region and allows to use special content elements inside of this region.<br/>
It also provides for summary error display for validation errors.<br/>
...
After minification jriapp.js has size about 307 KB. When it is gzipped, it is about 70 KB.
I recommend to use an ASP.NET MVC 4 bundling feature.
