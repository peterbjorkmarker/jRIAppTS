...
The Client part resembles  Microsoft Silverlight client development, only it is based on HTML (not XAML), and was written Microsoft Typescript language.
The framework was designed primarily for creating data centric Line of Business (LOB) applications which will work natively in internet browsers without the need for plugins .
...
The use of databindings and viewmodels resembles Microsoft Silverlight databindings style used in the XAML.

The framework was designed for obtaining a maximum convenience and performance, and for this sake it works in browsers which support ECMA Script 5.1 level of javascript and has features like native property setters and getters.

The supported browsers are Internet Explorer 9 and above, Mozilla Firefox 4+, Google Chrome 13+, and Opera 11.6+. 
Because the framework is primarily designed for developing LOB applications, the exclusion of antique browsers does not harm the purpose, and improves framework’s performance and ease of use.

The framework is distinguished from other frameworks available on the market by its full stack implementation of the features required for building real world  LOB applications in HTML5.
The strongly typed environment enables easy refactoring and syntax error checking at the development time.
...
It has also a special element view registered by the name dynacontent, which helps to create templated content regions on a page. The templates in these regions are easily switchable. This feature allows to create real world single page applications.
